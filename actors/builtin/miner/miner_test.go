...
func TestCommitments(t *testing.T) {
	...
	t.Run("invalid pre-commit rejected", func(t *testing.T) {
		...
		// Good commitment.
		actor.preCommitSector(rt, makePreCommit(100, challengeEpoch, deadline.PeriodEnd(), nil), big.Zero())
		...
		rt.ExpectAbort(exitcode.ErrIllegalArgument, func() {
			actor.preCommitSector(rt, makePreCommit(111, challengeEpoch, deadline.PeriodEnd(), nil), big.Zero())
		})

		// Expires before current epoch
		rt.SetEpoch(deadline.PeriodEnd() + 1)
		rt.ExpectAbort(exitcode.ErrIllegalArgument, func() {
			actor.preCommitSector(rt, makePreCommit(112, challengeEpoch, deadline.PeriodEnd(), nil), big.Zero())
		})

		// Expires not on period end
		rt.SetEpoch(precommitEpoch)
		rt.ExpectAbort(exitcode.ErrIllegalArgument, func() {
			actor.preCommitSector(rt, makePreCommit(113, challengeEpoch, deadline.PeriodEnd()-1, nil), big.Zero())
		})
...
func TestWindowPost(t *testing.T) {
	...
	t.Run("test proof", func(t *testing.T) {
		...
		_ = actor.commitAndProveSectors(rt, 1, expiration, big.Zero(), nil)
...
func TestProvingPeriodCron(t *testing.T) {
	...
	t.Run("first period gets randomness from previous epoch", func(t *testing.T) {
		...
		sectorInfo := actor.commitAndProveSectors(rt, 1, periodOffset+100*miner.WPoStProvingPeriod-1, big.Zero(), nil)
...
func TestExtendSectorExpiration(t *testing.T) {
	...
	deadline := st.DeadlineInfo(rt.Epoch())
	expiration := deadline.PeriodEnd() + 10*miner.WPoStProvingPeriod
	sectorInfo := actor.commitAndProveSectors(rt, 1, expiration, big.Zero(), nil)
...
func TestReportConsensusFault(t *testing.T) {
	...
	sectorInfo := actor.commitAndProveSectors(rt, 2, expiration, big.Zero(), dealIDs)
	_ = sectorInfo
...
func (h *actorHarness) commitAndProveSectors(rt *mock.Runtime, n int, expiration abi.ChainEpoch, pledge abi.TokenAmount, dealIDs [][]abi.DealID) []*miner.SectorPreCommitInfo {
